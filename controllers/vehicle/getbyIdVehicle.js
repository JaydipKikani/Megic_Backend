...
const getbyIdVehicle = async (req, res) => {
    try {
        ...
        const generalInfo = await GeneralInfo.findById(generalInfoId).populate('main_fuel second_fuel');
        if (!generalInfo) {
        ...
        const financialInfo = await FinancialInfo.findOne({ general_info: generalInfoId }).populate('finance_type');
